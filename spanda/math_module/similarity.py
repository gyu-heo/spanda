...
from spanda.util import indexing
...
def proj(v1, v2):
    ...
    ), f"v1.shape[0] must equal v2.shape[0]. Got v1.shape[0]={v1.shape[0]} and v2.shape[0]={v2.shape[0]}"
...
def vector_angle(v1, v2=None, mode="cosine_similarity"):
    ...
    if mode == "cosine_similarity":
        ...
        raise ValueError(
            f"mode must be 'cosine_similarity', 'deg', or 'rad'. Got {mode}"
        )
...
def orthogonalize(v1, v2, method="OLS", device="cpu", thresh_EVR_PCA=1e-15):
    ...
    if method == "serial":
        ...
        X = torch.cat(
            (pcsat, torch.ones((pcsat.shape[0], 1), dtype=pcsat.dtype, device=device)),
            dim=1,
        )
        theta = torch.linalg.inv(X.T @ X) @ X.T @ (v1 - v1_means)
        y_rec = X @ theta
        v1_orth = (
            v1 - y_rec + X[:, -1][:, None] * theta[-1]
        )  ## this adds back the bias term

    v1_orth = v1_orth + v1_means

    EVR = 1 - (torch.var(v1_orth, dim=0) / torch.var(v1, dim=0))
    EVR_total = 1 - (
        torch.sum(torch.var(v1_orth, dim=0), dim=0)
        / torch.sum(torch.var(v1, dim=0), dim=0)
    )
...
def EV(y_true, y_pred):
    ...
    EV = 1 - np.sum((y_true - y_pred) ** 2, axis=0) / np.sum(
...
def pairwise_similarity(v1, v2=None, method="pearson", ddof=1):
    ...
    if method in ["pearson", "R"]:
        ...
        v1_ms = v1 - np.mean(v1, axis=0)  # 'mean subtracted'
        v2_ms = v2 - np.mean(v2, axis=0)
        output = (v1_ms.T @ v2_ms) / np.sqrt(
            np.sum(v1_ms**2, axis=0, keepdims=True).T
            * np.sum(v2_ms**2, axis=0, keepdims=True)
        )
    if method == "cosine_similarity":
        output = (v1 / (np.linalg.norm(v1, axis=0, keepdims=True))).T @ (
...
def batched_covariance(X, batch_size=1000, device="cpu"):
    ...
    """
    X_dl1 = list(
        indexing.make_batches(
            np.arange(X.shape[1]), batch_size=batch_size, return_idx=True
        )
    )
    X_dl2 = list(
        indexing.make_batches(
    ...
    for ii, (X_batch_i, idx_batch_i) in enumerate(
        tqdm(X_dl1, total=n_batches, leave=False, desc="outer loop")
    ):
        for jj, (X_batch_j, idx_batch_j) in enumerate(
            tqdm(X_dl2, total=n_batches, leave=False, desc="inner loop")
        ):
            ...
            X_cov[
                idx_batch_i[0] : idx_batch_i[-1], idx_batch_j[0] : idx_batch_j[-1]
            ] = (x_t @ x)
    return X_cov
...
def batched_matrix_multiply(X1, X2, batch_size1=1000, batch_size2=1000, device="cpu"):
    ...
    """
    X1_dl = indexing.make_batches(
        np.arange(X1.shape[1]), batch_size=batch_size1, return_idx=True
    )
    X2_dl = indexing.make_batches(
        np.arange(X2.shape[1]), batch_size=batch_size2, return_idx=True
    )
    ...
    for ii, (X_batch_i, idx_batch_i) in enumerate(
        tqdm(X1_dl, total=n_batches1, leave=False, desc="outer loop")
    ):
        for jj, (X_batch_j, idx_batch_j) in enumerate(X2_dl):
            ...
            Y[idx_batch_i[0] : idx_batch_i[-1], idx_batch_j[0] : idx_batch_j[-1]] = (
                x1_t @ x2
            )
    return Y
...
def best_permutation(mat1, mat2, method="pearson"):
    ...
    for ii in range(len(ind1)):
        ...
        if method == "cosine_similarity":
            sim_matched[ii] = pairwise_similarity(
                mat1[:, ind1[ii]], mat2[:, ind2[ii]], "cosine_similarity"
            )
...
def self_similarity_pairwise(mat_set, method="pearson"):
    ...
    for i_combo, combo in enumerate(combos):
        (
            corr_avg[i_combo],
            corr_matched[:, i_combo],
            ind1[:, i_combo],
            ind2[:, i_combo],
        ) = best_permutation(mat_set[:, :, combo[0]], mat_set[:, :, combo[1]], method)
    # print(corr_avg)
...
def maximum_directed_matching(weights, partition):
    ...
    assert np.all(
        partition[sources] < partition[targets]
    ), "Invalid weights/partition combination"
